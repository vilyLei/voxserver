...
def renderingStart():
    ...
    print("####### modelRendering outputResolution: ", imgW, imgW)
    ...
    if routput.bgTransparent:
        renderer.image_settings.file_format='PNG'
        if routput.path == "":
            renderer.filepath = routput.taskRootDir + "bld_rendering.png"
        else:
            if "." in routput.path:
                renderer.filepath = routput.path
            else:
                renderer.filepath = routput.path + "bld_rendering.png"
    else:
        renderer.image_settings.file_format='JPEG'
        if routput.path == "":
            renderer.filepath = routput.taskRootDir + "bld_rendering.jpg"
        else:
            if "." in routput.path:
                renderer.filepath = routput.path
            else:
                renderer.filepath = routput.path + "bld_rendering.jpg"
    #################################################################################
...
def startupSys(argv):
    if "--" in argv:
        ...
        if len(argv) > 0:
            ...
            renderingStart()
            lossTime = time.perf_counter() - beginTime
            print("####### modelRendering renderingStart ops lossTime: ", lossTime)
        else:
            argv = []

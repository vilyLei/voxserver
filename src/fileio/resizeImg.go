...
func main() {
	// ResizeImgAndSave("./", "color_02.jpg", 128, 128)
	ResizeImgAndSave("./", "cat01.png", 128, 128)
}
func main1() {

	// 打开图像文件
	file, err := os.Open("./color_02.jpg")
	if err != nil {
		panic(err)
	}
	defer file.Close()
	...
	// 缩小图像
	smallImg := resize(img, 128, 128)

	// 创建 JPEG 图像文件
	out, err := os.Create("./color_02_mini.jpg")
	if err != nil {
		panic(err)
	}
	defer out.Close()
	...
	// 创建 PNG 图像文件
	out, err = os.Create("example_small.png")
	if err != nil {
		panic(err)
	}
	defer out.Close()

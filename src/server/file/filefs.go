...
func getHeaderValueByKey(h http.Header, key string) string {
	if v := h[key]; len(v) > 0 {
		return v[0]
	}
	return ""
...
func hasHeaderKey(h http.Header, key string) bool {
	_, ok := h[key]
	return ok
}
...
func checkIfMatch(w http.ResponseWriter, r *http.Request) condResult {
	...
	for {
		...
		if etagStrongMatch(etag, getHeaderValueByKey(w.Header(), "Etag")) {
			return condTrue
		}
		im = remain
	}
...
func checkIfNoneMatch(w http.ResponseWriter, r *http.Request) condResult {
	inm := getHeaderValueByKey(r.Header, "If-None-Match")
	if inm == "" {
		return condNone
	}
	buf := inm
	for {
		...
		if etagWeakMatch(etag, getHeaderValueByKey(w.Header(), "Etag")) {
			return condFalse
		}
		buf = remain
	}
...
func checkIfRange(w http.ResponseWriter, r *http.Request, modtime time.Time) condResult {
	...
	ir := getHeaderValueByKey(r.Header, "If-Range")
	if ir == "" {
		return condNone
	}
	etag, _ := scanETag(ir)
...
func checkPreconditions(w http.ResponseWriter, r *http.Request, modtime time.Time) (done bool, rangeHeader string) {
	...
	rangeHeader = getHeaderValueByKey(r.Header, "Range")
